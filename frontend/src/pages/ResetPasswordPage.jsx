...
const ResetPasswordPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch(`http://localhost:8000/api/reset-password/reset/${token}`, {
            method: 'POST',
        ...
        if (response.ok) {
            ...
            setMessage("Succès ! Votre mot de passe est mis à jour.");
    ...
    <div className="d-flex align-items-center justify-content-center" style={{ minHeight: "80vh" }}>
        <div className="p-5 shadow-lg" style={{ 
            ...
            <h2 className="text-white text-center fw-bold mb-4">Nouveau mot de passe</h2>
            <p className="text-white-50 text-center mb-4 small">
                Choisissez un mot de passe robuste pour sécuriser votre compte.
            </p>

            {message && <div className="alert alert-success border-0 text-center small" style={{ backgroundColor: "rgba(40, 167, 69, 0.2)", color: "#2ecc71" }}>{message}</div>}
            {error && <div className="alert alert-danger border-0 text-center small" style={{ backgroundColor: "rgba(220, 53, 69, 0.2)", color: "#e74c3c" }}>{error}</div>}
